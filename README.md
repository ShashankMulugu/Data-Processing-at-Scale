...
### CSE-511-Project-Phase-2 
This project was done by "Group 4" as part of the CSE 511 course project.
Team Members:
Srinivas Shashank Mulugu, 
Monica Selvam, 
Magesh Sridhar, 
Akshay Ganesan, 
Amitesh Sridharan, and
...
###How to Run the Code On IntelliJ IDEA IDE:
Follow these steps exactly:
1. Download the template zip from the Github link: https://github.com/jiayuasu/CSE512-Project-Hotspot-Analysis-Template
2. IMPORTANT: Make sure to save everything in a path with no spaces. Eg: C:\cse511\project\phase2 
3. IMPORTANT: Install Java JDK 8 on a path with no spaces. It will NOT work otherwise. Eg: C:\Java\JDK and C:\Java\JRE. This the problem that we have faced during the initial set-up because Spark simply doesn't start a job if Java JDK is installed on a path with spaces.  
4. Install the IntelliJ IDEA IDE. 
5. Install the Scala Plug-in and sbt plug-in.
6. After that is done open the template installed earlier on the IDE through File->Open
7. It usually takes some time for the IDE to set everything up after all that, you go to the Project root\build.sbt
8. Look at the section for library dependencies and change spark-core to 2.4.5 and also spark-sql to 2.4.5.
9. The IDE will ask you import changes in the bottom right corner and you click it.
10. Insert your code.
11. Build the project using the build (hammer) icon on the top. 
12. Go to the sbt-shell icon is on the bottom-left in the IDE.
13. Type 'clean assembly' and hit enter.
14. A .JAR file is generated which can be found in .\target\scala-2.11\
15. Open a terminal which is also available in the IDE.
16. Type the following command for windows:
C:\Spark\spark-2.4.5-bin-hadoop2.7\bin\spark-submit .\target\scala-2.11\CSE512-Hotspot-Analysis-Template-assembly-0.1.0.jar .\test\output hotzoneanalysis .\src\resources\point-hotzone.csv .\src\resources\zone-hotzone.csv hotcellanalysis .\src\resources\yellow_trip_sample_100000.csv
17. For any UNIX based systems type:
./bin/spark-submit /CSE512-Project-Hotspot-Analysis-Template/target/scala-2.11/CSE512-Project-Hotspot-Analysis-Template-assembly-0.1.0.jar test/output hotzoneanalysis src/resources/point-hotzone.csv src/resources/zone-hotzone.csv hotcellanalysis src/resources/yellow_tripdata_2009-01_point.csv
(Not the exact command)
18. The command format is:
<spark-submit path ==> can be found in Spark Bin> <Path to the assembly JAR> <Output Path> hotzoneanalysis <data 1> <data 2> hotcellanalysis <data> 
19. The generated results are available in the 'results' folder.